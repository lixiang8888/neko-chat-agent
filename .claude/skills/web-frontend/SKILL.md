---
name: web-frontend
description: 给猫娘agent 做 Web 前端（浏览器界面、聊天页、GalGame 演出页、SSE 流式对接、存档列表）时必须先读这份。当用户说「加个网页 / Web UI / 前端 / 界面 / 页面 / 聊天框 / 浏览器里跑 / 部署成网页 / 做个 GalGame 界面」时使用。包含后端对接契约、四条会踩的坑、目录约定与视觉方向。
---
